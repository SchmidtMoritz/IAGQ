...
def grad_shotslist():
    ...
    fd = FiniteDifferenzen(
        ansatz, observable, [0, fd_h], "simulator", label=f"FD {fd_h}"
    )
    bd = FiniteDifferenzen(
        ansatz, observable, [0, -bd_h], "simulator", label=f"BD {bd_h}"
    )
    cd = FiniteDifferenzen(
        ansatz, observable, [-cd_h, cd_h], "simulator", label=f"CD {cd_h}"
    )
    ps = ParameterShift(ansatz, observable, "simulator")
    gps = GeneralParameterShift(ansatz, observable, gps_gamma, "simulator")
    spsa = GradientSampleEstimator(
        SPSAGradient(ansatz, observable, spsa_h, "simulator"), spsa_count
    )
    ...
    compare_gradients_shotslist(
        gradient_list, point, shots_list, parameter_indices, metric
    )
